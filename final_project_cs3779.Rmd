...
# Colored Area based on Number of Stops
frisks_by_prec = 
  stop_frisk_df %>% 
  group_by(precinct) %>% 
...
nypd_prec  = 
  inner_join(nypd_prec, frisks_by_prec, by="precinct")

pal <- colorNumeric("YlOrRd", domain = range(frisks_by_prec$totalfrisks))

nypd_prec =
nypd_prec %>% 
  mutate(
    tfc = pal(totalfrisks),
    label = str_c("Precinct: ", precinct, "<br/>", "Total Stops in 2016: ", totalfrisks) %>% map(htmltools::HTML)
  )
